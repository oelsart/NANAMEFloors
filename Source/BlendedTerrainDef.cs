...
public class BlendedTerrainDef : TerrainDef
{
  ...
  public override void PostLoad()
  {
    LongEventHandler.ExecuteWhenFinished(delegate
    {
      ...
      if (CoverGraphic is null)
      {
        var shader = CoverTerrain.Shader.GetBlendShader();
        CoverGraphic = GraphicDatabase.Get<Graphic_Terrain>(CoverTerrain.texturePath, shader, Vector2.one,
          CoverTerrain.DrawColor, Color.white, null, maskPath);
        ...
        if (CoverTerrain.customShader is not null && CoverTerrain.customShaderParameters is not null)
        {
      ...
      {
        CoverWaterDepthMaterial =
          new Material(ShaderDatabase.LoadShader(CoverTerrain.waterDepthShader).GetBlendShader())
          {
            renderQueue = 2000 + CoverTerrain.renderPrecedence
          };
      ...
      if (ModsConfig.BiotechActive && CoverGraphicPolluted == null &&
          (!CoverTerrain.pollutionOverlayTexturePath.NullOrEmpty() || !CoverTerrain.pollutedTexturePath.NullOrEmpty()))
      {
        ...
        CoverGraphicPolluted = GraphicDatabase.Get<Graphic_Terrain>(
          CoverTerrain.pollutedTexturePath ?? CoverTerrain.texturePath, shader, Vector2.one, CoverTerrain.DrawColor,
        ...
        if ((CoverTerrain.pollutionShaderType != null || CoverTerrain.customShader != null) &&
            CoverTerrain.customShaderParameters != null)
        {
  ...
  public BlendedTerrainDef Rotated(RotationDirection direction)
  {
    ...
    var terrainMask2 =
      new TerrainMask(terrainMask.maskTextureName, terrainMask.baseTerrain, terrainMask.coverTerrain, rot);
